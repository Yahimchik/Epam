...
public class ParallelMatrixSub implements Command {
    private GenericMatrix<Number> first;
    private GenericMatrix<Number> second;

    public ParallelMatrixSub(GenericMatrix<Number> first, GenericMatrix<Number> second) {
        this.first = first;
        this.second = second;
    }
    ...
    public void execute() throws ControllerException, InterruptedException {
        ParallelMatrixProduct product = new ParallelMatrixProduct(
                ...
                first, second);
        System.out.println(product.parallelSub());
    }
}
