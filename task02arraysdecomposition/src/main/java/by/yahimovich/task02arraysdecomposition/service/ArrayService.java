            ...
            while (gap >= 1) {
                for (int right = 0; right < genericArray.length(); right++) {
                    for (int index = right - gap; index >= 0; index -= gap) {
                        if (genericArray.getElement(index).doubleValue() <
                                genericArray.getElement(index + gap).doubleValue()) {
            ...
            for (int step = genericArray.length() / 2; step > 0; step /= 2) {
                for (int i = step; i < genericArray.length(); ++i) {
                    for (int j = i - step; j >= 0; j -= step) {
                        if (genericArray.getElement(j).doubleValue() >
                                genericArray.getElement(j + step).doubleValue()) {
